...
The ContextBuilder either produces a complete authorized package or reports a context insufficiency. It cannot hide a required conflicting source to fit the budget. Record a digest of the actual material supplied to the model so review coverage is auditable. Paper composition uses a role-specific projection: claim/evidence identity, the accepted argument, and every bibliography entry remain present, while repeated provenance envelopes and bounded background excerpts are reduced deterministically. The actual projection mode and conservative token estimate are recorded beside the writer attempt. A request that still cannot fit is a typed local context-budget failure, not a generic model retry; the Composer preserves the budget evidence and does not replay the same packet.

Writer response shape is an adapter concern, not a scientific acceptance gate. The runtime may normalize a harmless wrapper, missing schema/citation metadata, section or unit identifiers, Markdown headings, and unit-kind labels into the pinned manuscript structure. It must not invent missing observations, evidence, or interpretation to do so. Content completeness, depth, argument projection, citation binding, and independent review remain substantive gates; a response that contains the required scientific content is not rejected merely because its transport shape differs.
