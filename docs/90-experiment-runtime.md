...
9. Emit and adopt `results-package-2` only after the replay, deterministic recalculation, and model assessment all accept. The package also carries the pre-analysis design reference, the optional analysis summary, and a `quality_admission` decision when a quality contract is configured. A missing or incomplete analysis is retained as an explicit Methods work order; it does not invalidate otherwise reproducible raw observations. External publication remains outside this command.
...
The first runtime supports one frozen local-program experiment, exact deterministic replay, bounded image assets, two or more same-model procedural review roles, and one separate deterministic calculator. A quality contract now makes the distinction between replayability and substantive study design explicit: a reproducible result can be adopted with a `quality_admission` hold and routed to Methods for the missing analysis, while the paper gate still refuses release until the floor is met. The runtime still does not resume an interrupted experiment, schedule a parameter sweep across several machines, preserve full sample matrices unless the study program emits them, provide statistically independent model reviewers, or establish correctness against expert adjudication. Those remain explicit follow-on evaluation and scaling tasks, not hidden fallbacks.
