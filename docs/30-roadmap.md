...
> **Version:** v0.8 · **Date:** 2026-09-10 · **Status:** revised implementation plan; no runtime milestones are claimed complete.
...
|---|---|---|---|---|
| P0 | Freeze a testable contract, not every future feature | This document set; fixture definitions; explicit defaults/open configuration | Schemas and ownership agree; first acceptance contract/fixtures selected; no unresolved live-operation authority | Revised blueprint delivered; approval/configuration pending |
| P1 | Durable, adaptive improvement loop | Structured units/scoped changes; artifact/task/event core; parallel approaches; renewable capacity; independent verification; checkpoints; remote runner; initial live web adapters | Recovery, scoped-edit, review, and retrieval tests pass; stagnation redirects work; exact progress and retained results are inspectable | In progress: durable core + structured-change + review/issue + capacity/checkpoint slices landed (T01–T03, T04–T05, T06–T10, T13, T41–T42, T45, T52, T56–T58 passing); bounded paragraph runner and model protocol adapters implemented; live Crossref/MCP retrieval verified; external GPU paragraph production, verification, and adoption validated on a synthetic fixture |
| P2 | Four-department paper MVP | Supplied-results intake → survey → validated argument → reviewed manuscript → rendered candidate | One real case completes with traceable evidence and no unwaived integrity blocker; human approves exact output | Not implemented |
...
| P4 | Prove Score generality | Second deliverable, documented custom-Score interface | Report/proposal produced using existing department contracts and no department-code rewrite | Deferred |
...
Implement typed records and a fixture runner, then immutable objects with transactional task/events, leases, and messages. Build parallel candidate work, independent verification/selection, allocation decisions, and native wall-clock checkpoints as one vertical slice. Verify authority/recovery invariants, then connect an authorized external GPU endpoint and exercise cancellation, renewal, and stale outputs. Add a minimal retrieval adapter with SearchCampaign/QueryRecord/DiscoveryRecord/SourceCapture before broadening provider coverage.

The artifact core includes stable units and purpose-scoped change application from the outset. The initial retrieval milestone includes actual scholarly identity/search, source capture, and general web access; then broaden to OA/citation/repository routes and a useful MCP service. Verify the full discovery-to-evidence-to-local-repair path, not just successful HTTP requests or plugin registration.

Bind all of this to a research-project instance with separate environment, capability authority, source/artifact scope, and task records. Activate practical operators for actual tool setup/use and independent verification as needed, then return them to idle. Shared GPU and provider capacity requires aggregate accounting across projects without implicit data or credential sharing.

Run the adaptive fixture and baseline comparison before extending the remaining department roles and rendering. Complete a real paper case, then expand concurrency/provider coverage and run the held-out evaluation. Autonomous task proposal and a minimal evaluation harness are part of P1; large organizational scale comes after the measured loop works.

## 11. Package status

This checkout contains the design documents listed in `00-SSOT.md`, a README, the P0 freeze record, the P1 fixture set, a deployment-configuration template, one illustrative search YAML, and the initial `scisaurus` control-plane core (schema/store/events/messages/tasks/documents/changes + review/issues/protocol + CLI) with its fixture-test suite covering T01–T03, T04–T05, T06–T10, T13, and T56–T58. The bounded paragraph runtime adds configured GPU protocol calls, supervisory reassessment, independent review contexts, staged adoption, and live Crossref/MCP retrieval. It does not yet contain a general autonomous scheduler, dynamic Operations Cell orchestration, a full paper workflow, a historical document archive, or measured research-quality improvement. End-to-end external GPU inference and conditional paragraph adoption have been validated on a synthetic fixture; the measured scope is recorded in `60-paragraph-runtime.md`. The scenarios above remain future implementation acceptance requirements.
