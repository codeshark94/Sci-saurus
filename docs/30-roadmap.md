...
> **Version:** v0.9 · **Date:** 2026-09-10 · **Status:** implementation plan with evidence scoped to completed runtime slices.
...
| P4 | Broaden Score composition | Additional deliverables and richer custom-Score interfaces | New mission contracts reuse the same department and control-plane services | Broader composition deferred; first bounded non-paper Score and executable policy checker implemented in P1 |
| P5 | New scientific analysis and experiments | Analysis/experiment scope, method execution, and scientific result-validation contracts | Authorized experiment execution, isolation, provenance, cancellation, and domain-validation tests pass | Deferred; practical program operation already belongs to v1 |
...
Core paragraph/grant/manifest and adapter-state scenarios start in P1. Rich paper rendering and additional providers complete in P2. These are future conformance requirements, not tests already implemented.
...
3. **Resume interrupted surveys safely.** Reconstruct the search frontier, source/map basis, accepted dependencies, outstanding calls, and resource accounting from durable records. Revalidate capabilities and current evidence before dispatch; late results must retain their uncertainty and cannot repeat committed effects.
4. **Complete the paper pipeline.** Connect supplied-results intake, accepted surveys, claim and argument checks, scoped manuscript composition, rendered-PDF review, and exact release artifacts through the existing general-purpose services. A bounded survey or favorable model verdict does not establish this complete workflow.

Implement ResultsPackage ingestion and gap reporting before generation. Add the Web Intelligence Fabric: SearchCampaign planning, provider-neutral retrieval adapters, QueryRecord/DiscoveryRecord logging, source acquisition, citation/author/project chasing where supported, multilingual query support, coverage reporting, and discovery→evidence promotion. Do not fabricate a reference card for empty queries and do not treat search snippets as scientific evidence. Implement evidence records, claim links, counterevidence, and exact source/result locators.

Activate Research, Strategy, Methods, and Editorial with one designated adversary each. Give every department task-scoped direct web discovery rights, while Research owns canonical scientific source capture/catalog promotion. Give each adversary an independently reserved retrieval tranche and require a separate query route for material novelty/validity challenges. Preserve blind initial review, producer rebuttal, independent contested adjudication, and verified closure across departments. Methods reviews supplied procedures/results only; it requests missing analyses instead of running them implicitly.

Use the paper Score as a controlled milestone sequence with explicit in-scope rework. Generate paragraph and structural-unit candidates with shared terminology/evidence contracts and narrow grants, integrate verified ChangeSets through a manifest, and compile in a restricted sandbox. Keep unit/source maps and inspect the rendered result for layout defects in addition to checking the compiler log. Add the API/citation/OA/repository and useful MCP capabilities in `50-web-intelligence-integration.md` under the shared adapter contract.
...
**P5 — New scientific execution.** Extend the existing project Operations Cell and execution profiles to substantive analyses and experiments with explicit method, input, expected-result, and validation contracts. Capability inventory, suitable program installation/build, MCP/API connection, source processing, and document generation already operate in P1/P2. This phase adds scientific-result generation; it does not introduce the practical execution machinery for the first time.
...
1. **Durable resume and deadline replanning.** Reconstruct pending tasks, evidence dependencies, search frontiers, unknown external outcomes, accepted artifacts, cumulative usage, and deadlines from durable records. Revalidate before dispatch, preserve completed work, and explicitly revise scope when remaining work cannot fit. A process restart must neither lose progress nor repeat committed effects.
2. **Executable general-purpose project plans.** Extend fixed-stage Scores with versioned task dependencies, investigation/production/verification proposals, selective reopening, retained alternatives, and completion contracts. Reuse existing artifact, authority, review, and time services. Academic requirements apply only to the chosen mission.
3. **On-demand Operations capability acquisition.** Turn an actual task capability gap into a supported tool choice, authorized installation/build/connection, representative execution, and independently verified reusable binding. Configured adapter readiness already works; arbitrary tool selection and acquisition do not. Installation or discovery records alone cannot establish usable capability.
4. **Held-out judgment evaluation.** Build a frozen corpus and repeatable evaluation runner with blinded expert labels across paper and non-paper tasks. Measure unsupported acceptance, incorrect rejection, repair quality, justified abstention, human intervention, and elapsed time/resources. Include novelty-specific metrics for research Scores before unattended scientific use.
5. **Complete the flagship paper Score.** Connect supplied-results intake and missing-data reporting, accepted surveys, claim/argument structure, scoped writing, reference reconciliation, LaTeX compilation, rendered-PDF review, and exact release artifacts. Add alternate full-text discovery and stable source-span pointers where required. Preserve the survey and exact nomination behind each verdict.
...
This checkout includes the durable artifact/task/event core, scoped document changes, review/issue services, capacity/checkpoint accounting, and bounded paragraph/project/survey runners. The shared project runner also consumes a versioned bounded Score with generic output units, explicitly selected Crossref/MCP/local-program workloads, exact candidate checks, and optional result/time targets. The operating-policy example includes a real offline JSON-schema checker and inert runtime preparation. The survey runner adds OpenAlex discovery and citation expansion, configured MCP full text, immutable work analyses, and exact survey/nomination prerequisites for independent gap assessment.

External GPU paragraph and multi-paragraph milestones are recorded in `60-paragraph-runtime.md` and `65-project-runtime.md`. `70-scored-project-runtime.md` defines the generic Score implementation and its verification boundaries; `75-literature-survey-score.md` records the bounded survey workflow, local tests, and finite coverage and source-access limits. A general autonomous scheduler, arbitrary task graph execution, automatic tool installation, resumable survey execution, deployment, complete paper workflow, workflow-plugin system, and measured research-quality improvement remain outside this slice. Acceptance scenarios without corresponding implementation evidence remain roadmap requirements.
