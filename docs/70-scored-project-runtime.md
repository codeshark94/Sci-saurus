...
Ordinary preconfigured `local_program` declarations are **not an operating-system sandbox** and run with the host account's permissions. They select already authorized programs and do not authorize arbitrary model-generated commands or automatic installation. Capability Foundry programs are a separate fail-closed profile: their descriptor requires the macOS deny-by-default sandbox during admission, readiness, and every actual experiment execution; a host without that boundary cannot generate, register, or run them.
