...
def admit_program_candidate(candidate, *, execute, validate, review=None, replay_runs=3):
    ...
    if verdict.get("decision") != "accepted":
        raise ValidationError("independent recalculation did not accept the candidate")
    ...
    for item in recalculations:
        if not isinstance(item, dict) or item.get("matches") is not True \
                or not _finite(item.get("recalculated_value")):
            raise ValidationError("independent recalculation did not match every declared outcome")
    review_record = None
    ...
    return {
        ...
        "gates": ["static_scan", "deterministic_replay", "test_vector_digest",
                  "independent_recalculation"] + (["adversarial_review"] if review_record else []),
    }
