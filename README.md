...
For a fully autonomous free-topic mission, add a `topic_discovery` stage before the survey in a Composer workflow and set the survey as its downstream dependency. The topic stage samples recent OpenAlex records with an entropy-backed seed and response trace, asks the configured model for orthogonal testable directions, and selects one against the actual project capabilities. A journal-oriented topic→survey mission then runs an independent topic-maturity screen; thin executable questions are regenerated with a substantive change before the survey. A new mission gets a fresh seed and a resume reuses the persisted seed. Topic checkpoints are not reused unless the workflow explicitly sets `topic_reuse_allowed: true`; this prevents an old topic from silently becoming the next free exploration. Role-specific sampling profiles give topic and blind-search agents more exploratory temperature, while evidence mapping, arbitration, and journal review remain conservative; a model file may override them through `role_profiles`. The Composer projects the selected question and search strings into the survey automatically, so ordinary workflows do not need hand-written bindings for those fields. If the first literature assessment is insufficient, Composer expands the survey before redesigning the question; if the expanded evidence still cannot support it, or prior work already answers it, Composer holds the experiment, issues a scoped topic-refinement order, and reopens the topic and survey with the parent question and assessment attached. Set `time_policy.hard_seconds` to the mission wall (for example `36000` for ten hours), set the workflow retry policy to `mode: until_deadline`, and set the survey limits `repair_mode: until_deadline` when malformed model output must be repaired within the same wall. Research continuation is also deadline-governed by default; select `continuation_policy.mode: bounded` only for a small deterministic job that needs a finite re-entry budget. See [Composer runtime](docs/110-composer-runtime.md).
