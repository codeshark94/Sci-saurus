...
This repository contains design documents, the P0 freeze record, the selected P1 fixture set, and a deployment-configuration template. The P1 durable core slice is implemented under `scisaurus/` — a transactional control store with an event hash chain, immutable artifact publication with adoption CAS, a message bus (outbox, leases, fencing tokens, idempotent effects), the task/attempt lifecycle, and structured-change application (stable content units, document manifests, scoped EditGrants, protected spans, citation anchoring) — with acceptance tests T01–T03, T04, T05, T13, and T56–T58 passing. It does not yet contain a scheduler/runner, model adapter, retrieval adapters, review/issue records, a runnable paper workflow, or benchmark result. External GPU capacity is a design premise; endpoints, models, measured capacity, and data permissions remain deployment configuration.
